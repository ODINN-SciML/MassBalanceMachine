...
from regions.Switzerland.scripts.xgb_helpers import process_or_load_data, transform_df_to_seasonal, get_CV_splits
...
_default_train_glaciers = ['clariden',
    'oberaar', 'otemma', 'gietro', 'rhone', 'silvretta',
    'gries', 'sexrouge', 'allalin', 'corbassiere', 'aletsch',
    'joeri', 'basodino', 'morteratsch', 'findelen', 'albigna',
    'gorner', 'murtel', 'plainemorte', 'adler', 'limmern',
    'schwarzbach'
]

_default_additional_var = ['ALTITUDE_CLIMATE', 'ELEVATION_DIFFERENCE', 'POINT_ELEVATION', 'pcsr']
_default_vois_climate = ['t2m', 'tp', 'slhf', 'sshf', 'ssrd', 'fal', 'str', 'u10', 'v10']
_default_vois_topographical = ["aspect_sgi", "slope_sgi", "hugonnet_dhdt", "consensus_ice_thickness", "millan_v"]
_default_input = _default_additional_var+_default_vois_climate+_default_vois_topographical
...
def getTrainTestSets(target_train_glaciers, test_glaciers, params, cfg, process=False):
    ...
    data_glamos.drop(data_glamos[data_glamos.GLACIER == 'taelliboden'].index, inplace=True)


    vois_climate = params['model'].get('vois_climate', _default_vois_climate)
    vois_topographical = params['model'].get('vois_topographical', _default_vois_topographical)
    paths = {
    ...
    dataloader_gl = process_or_load_data(run_flag=process,
