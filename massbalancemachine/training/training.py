...
_default_log_dir = os.path.abspath(os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../logs/'))
...
def assessOnTest(log_dir, model, geodataloader_test):
    ...
    indAnnual = torch.argwhere(periodAll==geodataloader_test.periodToInt['annual'])[:,0]
    indWinter = torch.argwhere(periodAll==geodataloader_test.periodToInt['winter'])[:,0]
    predAnnual = predAll[indAnnual]
    targetAnnual = targetAll[indAnnual]
    predWinter = predAll[indWinter]
    targetWinter = targetAll[indWinter]
    mse_annual, rmse_annual, mae_annual, pearson_corr_annual, r2_annual, bias_annual = scores(predAnnual, targetAnnual)
    mse_winter, rmse_winter, mae_winter, pearson_corr_winter, r2_winter, bias_winter = scores(predWinter, targetWinter)
    ...
    return {
        'mse': mse.item(), 'rmse': rmse.item(), 'mae': mae.item(), 'pearson': pearson_corr.item(), 'r2': r2.item(), 'bias': bias.item(),
        'mse_annual': mse_annual.item(), 'rmse_annual': rmse_annual.item(), 'mae_annual': mae_annual.item(), 'pearson_annual': pearson_corr_annual.item(), 'r2_annual': r2_annual.item(), 'bias_annual': bias_annual.item(),
        'mse_winter': mse_winter.item(), 'rmse_winter': rmse_winter.item(), 'mae_winter': mae_winter.item(), 'pearson_winter': pearson_corr_winter.item(), 'r2_winter': r2_winter.item(), 'bias_winter': bias_winter.item(),
    }
...
def loadBestModel(log_dir, model):
    ...
    for i, f in enumerate(files):
        ...
        if bestVal is None:
            best = i
            bestVal = val
        elif val==bestVal: raise Exception("Two models have exactly the same validation score. Please update the implementation of loadBestModel to support this case.")
...
def train_geo(
    ...
    if log_suffix != '': log_suffix = '_' + log_suffix
    log_dir = trainCfg.get('log_dir', os.path.join(_default_log_dir, f'geo_{run_name}{log_suffix}', ))
    ...
    for epoch in tqdm(range(Nepochs), desc="Epochs", position=0, leave=True, ncols=nColsProgressBar):
        ...
        if freqVal:
            ...
            with torch.no_grad():
                ...
                writer.add_scalar('Pearson_annual/val', pearson_corr_annual.item(), epoch)
                writer.add_scalar('Pearson_winter/val', pearson_corr_winter.item(), epoch)
                ...
                scalarBestModelCriterion = statsVal[bestModelCriterion][-1] if len(statsVal[bestModelCriterion])>0 else np.nan
                higherBetterCriterion = 1 if any(crit in bestModelCriterion for crit in _maxCriterion) else -1
                model_path = os.path.join(log_dir, f"model_epoch{epoch}_{bestModelCriterion}{scalarBestModelCriterion:.4f}.pt")

                if len(top_models) < 5 or scalarBestModelCriterion < higherBetterCriterion * top_models[0][0]:
                    ...
                    heapq.heappush(top_models, (higherBetterCriterion * scalarBestModelCriterion, model_path))
        ...
        # Show progress bar
        tqdm.write(f"[Epoch {epoch+1} / {Nepochs}] Avg training loss: {avg_training_loss:.3f}, Val loss: {loss:.3f}, RMSE: {rmse:.3f}, Pearson: {pearson:.3f}")

    # Close TensorBoard
    writer.close()

    return {'training': statsTraining, 'validation': statsVal, 'misc': {'log_dir': log_dir}}
