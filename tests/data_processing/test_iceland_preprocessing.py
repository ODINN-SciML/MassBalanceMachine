...
def test_data_preprocessing():
    target_raw_data_fname = './notebooks/example_data/iceland/files/iceland_stake_dataset.csv'
    data = pd.read_csv(target_raw_data_fname)
    assert data.shape == (19, 18)

    assert all(data.keys()==['stake', 'yr', 'd1', 'd2', 'd3', 'lat', 'lon', 'elevation', 'rhow',
       'rhos', 'bw_stratigraphic', 'bs_stratigraphic', 'ba_stratigraphic',
       'bw_floating_date', 'bs_floating_date', 'ba_floating_date', 'GLIMSId',
       'Name'])
