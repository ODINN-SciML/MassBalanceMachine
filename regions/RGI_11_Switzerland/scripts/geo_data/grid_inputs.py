...
from regions.RGI_11_Switzerland.scripts.utils import *
from regions.RGI_11_Switzerland.scripts.geo_data.svf import open_and_merge_svf
from regions.RGI_11_Switzerland.scripts.geo_data.geodata import *
...
def create_masked_glacier_grid(path_RGIs, rgi_gl):
    ...
    # --- Float mask: 1.0 inside, NaN outside ---
    glacier_mask = np.where(gl_mask_bool, 1.0, np.nan)

    # --- Apply mask to core variables ---
    ds = ds.assign(
        masked_slope=xr.DataArray(
            glacier_mask * ds["slope"].values, dims=ds["slope"].dims
        )
    )
    ds = ds.assign(
        masked_elev=xr.DataArray(glacier_mask * ds["topo"].values, dims=ds["topo"].dims)
    )
    ds = ds.assign(
        masked_aspect=xr.DataArray(
            glacier_mask * ds["aspect"].values, dims=ds["aspect"].dims
        )
    )
    ds = ds.assign(
        masked_dis=xr.DataArray(
            glacier_mask * ds["dis_from_border"].values, dims=ds["dis_from_border"].dims
        )
    )

    if "hugonnet_dhdt" in ds:
        ds = ds.assign(
            masked_hug=xr.DataArray(
                glacier_mask * ds["hugonnet_dhdt"].values, dims=ds["hugonnet_dhdt"].dims
            )
        )
    if "consensus_ice_thickness" in ds:
        ds = ds.assign(
            masked_cit=xr.DataArray(
                glacier_mask * ds["consensus_ice_thickness"].values,
                dims=ds["consensus_ice_thickness"].dims,
            )
        )
    if "millan_v" in ds:
        ds = ds.assign(
            masked_miv=xr.DataArray(
                glacier_mask * ds["millan_v"].values, dims=ds["millan_v"].dims
            )
        )

    # --- Derive indices from the same boolean mask ---
