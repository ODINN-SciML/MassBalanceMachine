...
# --- small helpers ---
def _drop_summer(df: pd.DataFrame) -> pd.DataFrame:
    return df[df.PERIOD != "summer"] if "PERIOD" in df.columns else df


def _post_france(df: pd.DataFrame) -> pd.DataFrame:
    df = _drop_summer(df)
    if "GLACIER" in df.columns:
        df["GLACIER"] = df["GLACIER"].map(FR_gl_name)

    # drop GLACIER_ZONE and DATA_MODIFICATION, which are not consistent across datasets and we don’t use in the paper
    df.drop(
        columns=["GLACIER_ZONE", "DATA_MODIFICATION"], inplace=True, errors="ignore"
    )
    return df


def _post_switzerland(df: pd.DataFrame) -> pd.DataFrame:
    df = _drop_summer(df)
    # filter glaciers with PCSR availability (your existing list)
    glDirect = [
        "adler",
        "albigna",
        "aletsch",
    ...
    if "GLACIER" in df.columns:
        df = df[df.GLACIER.isin(glDirect)]

    # Rename aspect_sgi and slope_sgi to aspect and slope for consistency with other datasets
    if "aspect_sgi" in df.columns:
        # drop the old aspect column if it exists, since it’s not consistent across datasets
        df.drop(columns=["aspect"], inplace=True, errors="ignore")
        df = df.rename(columns={"aspect_sgi": "aspect"})
    if "slope_sgi" in df.columns:
        df.drop(columns=["slope"], inplace=True, errors="ignore")
        df = df.rename(columns={"slope_sgi": "slope"})
    if "topo_sgi" in df.columns:
        df.drop(columns=["topo"], inplace=True, errors="ignore")
        df = df.rename(columns={"topo_sgi": "topo"})

    # drop asvf and opnsvf, which are not consistent across datasets and we don’t use in the paper
    df.drop(columns=["asvf", "opns"], inplace=True, errors="ignore")
    return df


def _post_default(df: pd.DataFrame) -> pd.DataFrame:
    df.drop(columns=["DATA_MODIFICATION"], inplace=True, errors="ignore")
    return _drop_summer(df)


# --- registry: code -> file + postprocessor ---
STAKES_SPECS = {
    "SJM": {
        "relpath": Path("WGMS") / "Svalbard" / "csv" / "SJM_wgms_dataset_all.csv",
        "post": _post_default,
    },
    "NOR": {
        "relpath": Path("WGMS") / "Norway" / "csv" / "NOR_wgms_dataset_all.csv",
        "post": _post_default,
    },
    "ISL": {
        "relpath": Path("WGMS") / "Iceland" / "csv" / "ISL_wgms_dataset_all.csv",
        "post": _post_default,
    },
    "FR": {
        "relpath": Path("WGMS") / "France" / "csv" / "FR_wgms_dataset_all.csv",
        "post": _post_france,
    },
    "CH": {
...
def load_stakes(cfg, code: str) -> pd.DataFrame:
    """
    Load stakes by country/subregion code, e.g. 'CH', 'FR', 'ISL', 'SJM', 'NOR'.
    """
    code = code.upper()
    if code not in STAKES_SPECS:
        raise KeyError(f"Unknown stakes code '{code}'. Known: {sorted(STAKES_SPECS)}")

    spec = STAKES_SPECS[code]
    fpath = Path(cfg.dataPath) / spec["relpath"]
    df = pd.read_csv(fpath)
    return spec["post"](df)


# Load by RGI region id (loads and concatenates subregions)
def codes_for_rgi_region(rgi_region: str) -> list[str]:
    rgi_region = str(rgi_region).zfill(2)
    reg = RGI_REGIONS[rgi_region]

    # if it has explicit subregions, use those; else use the region-level code
    if "subregions_codes" in reg and reg["subregions_codes"]:
        return [c.upper() for c in reg["subregions_codes"]]
    return [reg["code"].upper()]


def load_stakes_for_rgi_region(cfg, rgi_region: str) -> pd.DataFrame:
    codes = codes_for_rgi_region(rgi_region)
    frames = []
    for c in codes:
        if c in STAKES_SPECS:  # allow missing codes while you’re still adding datasets
            df = load_stakes(cfg, c)
            df["SOURCE_CODE"] = c
            df["RGI_REGION"] = str(rgi_region).zfill(2)
            frames.append(df)
        else:
            print(f"[warn] No stakes spec for code={c} (RGI {rgi_region})")
...
def resolve_experiment_region_codes(region_code: str, region_groups: dict) -> list[str]:
    """
    Expand an experiment region code into atomic source codes.
    """
    region_code = str(region_code).upper()
    return region_groups.get(region_code, [region_code])


def code_to_rgi_id(code: str) -> str:
    """
    Map a region code to its 2-digit RGI region id when possible.
    Grouped regions spanning multiple RGI regions return 'MULTI'.
    """
    code = str(code).upper()
...
def prepare_monthly_dfs_for_all_regions(
    ...
    only_codes_set = {str(c).upper() for c in only_codes} if only_codes else None
    test_glaciers_override = test_glaciers_override or {}
    test_glaciers_by_code = test_glaciers_by_code or {}
    ...
    def codes_from_key(k):
        """
        Decide which region codes should actually be processed.
        If a key is explicitly present in test_glaciers_by_code, process it as-is.
        """
        k_norm = normalize_key(k)

        if k_norm in test_glaciers_by_code:
            return [k_norm]

        if is_rgi_key(k_norm):
            rid2 = str(k_norm).zfill(2)
            return [c.upper() for c in codes_for_rgi_region(rid2)]

        return [k_norm]

    def should_recompute(rid2, code):
        if not run_flag:
            return False
        ...
        match_rid = (
            True
            if rid2 == "MULTI"
            else (only_rids_set is None or rid2 in only_rids_set)
        )
        match_code = only_codes_set is None or code in only_codes_set
        return match_rid and match_code

    jobs = []

    for region_key, df_region in dfs.items():
        if df_region is None or len(df_region) == 0:
            continue

        region_codes = codes_from_key(region_key)
        has_source = "SOURCE_CODE" in df_region.columns
        region_key_norm = normalize_key(region_key)

        for code in region_codes:
            rid2 = code_to_rgi_id(code)

            # grouped/final region: keep whole dataframe
            if code == region_key_norm and code in test_glaciers_by_code:
                df_sub = df_region.copy()

            # atomic slicing
            elif has_source:
                df_sub = df_region[
                    df_region["SOURCE_CODE"].astype(str).str.upper() == code
                ].copy()
            ...
            if len(df_sub) == 0:
                logging.warning(
                    f"[{region_key} -> {rid2}_{code}] No rows found after slicing, skipping."
                )
                continue

            jobs.append(
                {
                    "input_key": region_key,
                    "rid2": rid2,
                    "region_id_int": None if rid2 == "MULTI" else int(rid2),
                    "code": code,
                    "df_sub": df_sub,
                    "key": code if rid2 == "MULTI" else f"{rid2}_{code}",
                    "should_run": should_recompute(rid2, code),
                }
            )

    recompute_keys = [j["key"] for j in jobs if j["should_run"]]
    load_keys = [j["key"] for j in jobs if not j["should_run"]]
    ...
    elif len(recompute_keys) == len(jobs):
    ...
    for job in jobs:
        ...
        test_glaciers = test_glaciers_override.get(
            code,
            test_glaciers_by_code.get(code, []),
        )

        paths_ = paths.copy()

        def csv_path_for_code(cfg, code):
            code = str(code).upper()

            if code in {"CEU", "USCA"}:
                return os.path.join(cfg.dataPath, path_PMB_WGMS_csv, code, "csv")

            rid2 = code_to_rgi_id(code)
            country = country_folder_for_code(rid2, code)

            path = os.path.join(cfg.dataPath, path_PMB_WGMS_csv, country, "csv")
            # make sure the path exists
            os.makedirs(path, exist_ok=True)
            return path

        paths_["csv_path"] = csv_path_for_code(cfg, code)
        country = code

        logging.info(
            f"Processing {key} (input={job['input_key']}, country={country}, run_flag={should_run})"
        )
