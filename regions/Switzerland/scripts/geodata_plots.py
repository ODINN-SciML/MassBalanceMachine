...
def scatter_geodetic_MB(df_all, hue='GLACIER', size=False):
    ...
    plot_scatter(df_all, hue, size, axs[1], "GLAMOS MB", rmse_glamos,
                 corr_glamos)
...
def plot_mass_balance(glacier_name, year, df_stakes,
    ...
    stakes_data_ann["Predicted_MB"] = stakes_data_ann.apply(
        lambda row: get_predicted_mb(lon_name, lat_name, row, ds_mbm_ann),
        axis=1)
    ...
    stakes_data_win["Predicted_MB"] = stakes_data_win.apply(
        lambda row: get_predicted_mb(lon_name, lat_name, row, ds_mbm_win),
        axis=1)
...
def plot_mass_balance_comparison(
    ...
    for _, row in df.iterrows():
        ...
        axs[1].hlines(y=row['mbm_geod'],
                      xmin=x_start,
                      xmax=x_end,
                      color=color_mbm,
                      linewidth=2)
        axs[1].hlines(y=row['glamos_geod'],
                      xmin=x_start,
                      xmax=x_end,
                      color=color_model2,
                      linewidth=2)
...
def plot_mass_balance_comparison_annual(glacier_name, year, cfg, df_stakes,
                                        path_distributed_mb, path_pred_xgb,
                                        path_pred_nn, get_glamos_func,
                                        get_pred_func, get_glamos_pred_func,
                                        load_grid_func, to_wgs84_func,
                                        apply_filter_func, get_colormaps_func):
    ...
    # GLAMOS file path
    file_ann = f"{year}_ann_fix_lv95.grid"
    grid_path_ann = os.path.join(cfg.dataPath, path_distributed_mb, 'GLAMOS',
    ...
    mbm_file_xgb = os.path.join(path_pred_xgb, glacier_name,
                                f"{glacier_name}_{year}_annual.zarr")
    ds_mbm_xgb = apply_filter_func(xr.open_dataset(mbm_file_xgb))

    mbm_file_nn = os.path.join(path_pred_nn, glacier_name,
                               f"{glacier_name}_{year}_annual.zarr")
    ...
    if not stakes_data_ann.empty:
        stakes_data_ann["Predicted_MB_XGB"] = stakes_data_ann.apply(
            lambda row: get_pred_func(lon_name, lat_name, row, ds_mbm_xgb),
            axis=1)
        stakes_data_ann["Predicted_MB_NN"] = stakes_data_ann.apply(
            lambda row: get_pred_func(lon_name, lat_name, row, ds_mbm_nn),
            axis=1)
        stakes_data_ann["GLAMOS_MB"] = stakes_data_ann.apply(
            lambda row: get_glamos_pred_func(lon_name, lat_name, row,
                                             ds_glamos_wgs84_ann),
            axis=1)

        stakes_data_ann.dropna(
            subset=["Predicted_MB_XGB", "Predicted_MB_NN", "GLAMOS_MB"],
            inplace=True)

    # Color scale limits
    vmin = min(ds_glamos_wgs84_ann.min().item(),
               ds_mbm_xgb.pred_masked.min().item(),
               ds_mbm_nn.pred_masked.min().item())
    vmax = max(ds_glamos_wgs84_ann.max().item(),
    ...
    if not stakes_data_ann.empty:
        sns.scatterplot(data=stakes_data_ann,
                        ...
                        s=25,
                        legend=False)
        rmse_glamos = mean_squared_error(stakes_data_ann.POINT_BALANCE,
                                         stakes_data_ann.GLAMOS_MB,
                                         squared=False)
        text_glamos = f"RMSE: {rmse_glamos:.2f},\nmean MB: {ds_glamos_wgs84_ann.mean().item():.2f},\nvar: {var_glamos:.2f}"
    else:
        text_glamos = f"mean MB: {ds_glamos_wgs84_ann.mean().item():.2f},\nvar: {var_glamos:.2f}"
    ...
    if not stakes_data_ann.empty:
        sns.scatterplot(data=stakes_data_ann,
                        ...
                        s=25,
                        legend=False)
        rmse_xgb = mean_squared_error(stakes_data_ann.POINT_BALANCE,
                                      stakes_data_ann.Predicted_MB_XGB,
                                      squared=False)

        text_xgb = (f"RMSE: {rmse_xgb:.2f},\n"
                    f"mean MB: {ds_mbm_xgb.pred_masked.mean().item():.2f},\n"
                    f"var: {var_xgb:.2f}")
    else:
        text_xgb = (f"mean MB: {ds_mbm_xgb.pred_masked.mean().item():.2f},\n"
                    f"var: {var_xgb:.2f}")
    ...
    if not stakes_data_ann.empty:
        sns.scatterplot(data=stakes_data_ann,
                        ...
                        s=25,
                        legend=False)
        rmse_nn = mean_squared_error(stakes_data_ann.POINT_BALANCE,
        ...
        text_nn = (f"mean MB: {ds_mbm_nn.pred_masked.mean().item():.2f},\n"
                   f"var: {var_nn:.2f}")
    axes[2].text(0.05,
                 0.15,
                 text_nn,
...
def plot_scatter_comparison(ax,
                            df,
                            glacier_name,
                            color_mbm,
                            color_glamos,
    ...
    ax.set_xlabel("Geodetic MB [m w.e.]", fontsize=12)
    ax.set_ylabel("Modeled MB [m w.e.]", fontsize=12)
    ax.set_title(f"{glacier_name.capitalize()} Glacier {title_suffix}",
                 fontsize=14)
    ax.legend(loc="upper left", fontsize=10)
...
def plot_mbm_vs_geodetic_by_area_bin(
    ...
    for i, area_bin in enumerate(unique_bins[:max_bins]):
        ...
        sns.scatterplot(
            ...
            ax=ax,
            s = 250,
            palette=sns.color_palette(get_cmap_hex(cm.batlow, 1+len(df_bin[glacier_col].unique()))),
        )
        
        #sns.color_palette("Paired",len(df_bin[glacier_col].unique()))

        # Identity line and axes formatting
        ax.axvline(0, color="grey", linestyle="--", linewidth=1)
        ax.axhline(0, color="grey", linestyle="--", linewidth=1)
        ax.grid(True, linestyle="--", linewidth=0.5)
        ax.set_xlabel(f"Observed geodetic MB [m w.e.]")
        ax.set_ylabel(f"Modelled MB [m w.e.]")
    ...
    plt.tight_layout()
    plt.subplots_adjust(bottom=-0.20)  # <-- Add this line to provide space for legends
    plt.show()


def plot_mass_balance_comparison_annual_glamos_nn(glacier_name, year, cfg, df_stakes,
                                                  path_distributed_mb, path_pred_nn,
                                                  get_glamos_func, get_pred_func,
    ...
    if not stakes_data_ann.empty:
        ...
        stakes_data_ann["GLAMOS_MB"] = stakes_data_ann.apply(
            ...
            axis=1)
        stakes_data_ann.dropna(
            subset=["Predicted_MB_NN", "GLAMOS_MB"],
            inplace=True)
    ...
    if not stakes_data_ann.empty:
        ...
        text_glamos = f"mean MB: {ds_glamos_wgs84_ann.mean().item():.2f},\nvar: {var_glamos:.2f}"
    axes[0].text(0.05, 0.15, text_glamos, transform=axes[0].transAxes,
    ...
    if not stakes_data_ann.empty:
        ...
        text_nn = (f"mean MB: {ds_mbm_nn.pred_masked.mean().item():.2f},\n"
                   f"var: {var_nn:.2f}")
    axes[1].text(0.05, 0.15, text_nn, transform=axes[1].transAxes,
                 ha='left', va='top', fontsize=18)

    plt.suptitle(f"{glacier_name.capitalize()} Glacier – Annual MB (GLAMOS vs NN, {year})",
                 fontsize=20)
    plt.tight_layout()
    plt.show()
