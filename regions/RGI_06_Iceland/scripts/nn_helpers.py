...
from regions.RGI_06_Iceland.scripts.plots import *
...
class SaveBestAtEpochs(Callback):
    ...
    def on_epoch_end(self, net, **kwargs):
        ...
        if valid_loss < self.best_score:
            ...
            self.best_state = {
                k: v.cpu().clone() for k, v in net.module_.state_dict().items()
            }
...
def evaluate_model_and_group_predictions(custom_NN_model, df_X_subset, y, cfg, mbm):
    ...
    dataset = mbm.data_processing.AggregatedDataset(
        cfg, features=features, metadata=metadata, targets=y
    )
    ...
    scores = {"score": score, "mse": mse, "rmse": rmse, "mae": mae, "pearson": pearson}
...
def PlotPredictionsCombined_NN_additional(
    grouped_ids,
    region_name="",
    include_summer=False,
    nticks=6,
    ...
    slope, intercept, r_value, p_value, std_err = stats.linregress(
        grouped_ids.target, grouped_ids.pred
    )
