...
def get_oggm_features(RGIID, parent_file_path='.././', working_dir='/data/oggm'):
    RGI = RGIID

    # TODO make this this an assert
    if not RGI:
    ...
    df_path = os.path.join(parent_path, 'data/files/Iceland_Stake_Data_Reprojected.csv')
    df_path_output = os.path.join(parent_path, 'data/files/Iceland_Stake_Data_T_Attributes.csv')

    # Read stake data CSV file
    df = pd.read_csv(df_path)

    # Get unique RGI IDs -> This should be agreed on before in the documentation
    ...
    # TODO: also document about the lat lon names in the data files
